# ============================================================================
# Generate a technical analysis using GitHub Copilot CLI (gh models).
# No API keys, no n8n, no Docker needed — just `gh` CLI authenticated.
#
# Prerequisites:
...
#   .\scripts\gh-models-preview.ps1 -Description "Add notifications" -Context "We use Firebase"
#   .\scripts\gh-models-preview.ps1 -Description "Migrate auth" -Template tech-migration
...
#
# Default model: anthropic/claude-4-opus (Claude Opus via GitHub Models)
#
# Available templates:
...
param(
    ...
    [string]$Model = "anthropic/claude-4-opus"
)

# --- Check dependencies ---
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
    Write-Host "Error: 'gh' CLI not found. Install GitHub CLI:" -ForegroundColor Red
    Write-Host "  https://cli.github.com/"
    exit 1
}

$ghModelsCheck = gh models --help 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Host "Error: 'gh models' extension not installed. Install it:" -ForegroundColor Red
    Write-Host "  gh extension install github/gh-models"
    Write-Host ""
    Write-Host "Then verify: gh models list"
    exit 1
}
...
Write-Host "  AI Technical Analysis (gh models CLI)" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "  Calling gh models run... (this takes 15-60 seconds)" -ForegroundColor Yellow
Write-Host ""

# --- Write prompt to temp file and call gh models ---
$TmpFile = [System.IO.Path]::GetTempFileName()
$UserPrompt | Out-File -FilePath $TmpFile -Encoding utf8

try {
    $RawResponse = Get-Content $TmpFile -Raw | gh models run $Model --system-prompt $Role 2>$null
    if ($LASTEXITCODE -ne 0) { throw "gh models run returned exit code $LASTEXITCODE" }
}
catch {
    Write-Host "Error: gh models run failed." -ForegroundColor Red
    Write-Host ""
    Write-Host "Troubleshooting:" -ForegroundColor Yellow
    Write-Host "  1. Check auth:       gh auth status"
    Write-Host "  2. Check extension:  gh models list"
    Write-Host "  3. Check model ID:   gh models list | Select-String claude"
    Write-Host "  4. Try manually:     echo 'hello' | gh models run $Model"
...
$md += "# $Title"
$md += ""
$md += "> Generated on $Date by [ai-confluence-pipeline](https://github.com/openmindednewby/ai-confluence-pipeline) via gh models CLI ($Model)"
$md += ""

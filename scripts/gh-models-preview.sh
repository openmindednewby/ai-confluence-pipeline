...
#   ./scripts/gh-models-preview.sh "Add user notifications" --context "We use Firebase"
#   ./scripts/gh-models-preview.sh "Migrate auth service" --template tech-migration
...
if ! gh models --help &> /dev/null 2>&1; then
  echo "Error: 'gh models' extension not installed. Install it:"
  echo "  gh extension install github/gh-models"
  echo ""
  echo "Then verify with: gh models list"
  exit 1
fi
...
get_schema() {
  case "$1" in
    new-feature|generic|legacy-technical|large-refactoring)
...
USER_PROMPT="Analyze the following and produce a detailed JSON response.
...
echo "============================================"
echo "  AI Technical Analysis (gh models CLI)"
echo "============================================"
echo ""
echo "  Model:       $MODEL"
...
echo ""
echo "  Calling gh models run... (this takes 15-60 seconds)"
echo ""

# --- Write prompt to temp file and call gh models ---
TMPFILE=$(mktemp)
echo "$USER_PROMPT" > "$TMPFILE"

RAW_RESPONSE=$(cat "$TMPFILE" | gh models run "$MODEL" --system-prompt "$ROLE" 2>/dev/null) || {
  echo "Error: gh models run failed."
  echo ""
  echo "Troubleshooting:"
  echo "  1. Check auth:      gh auth status"
  echo "  2. Check extension:  gh models list"
  echo "  3. Check model ID:   gh models list | grep claude"
  echo "  4. Try manually:     echo 'hello' | gh models run $MODEL"
  rm -f "$TMPFILE"
  exit 1
}
rm -f "$TMPFILE"
...

{
  echo "# ${TITLE}"
  echo ""
  echo "> Generated on ${DATE} by [ai-confluence-pipeline](https://github.com/openmindednewby/ai-confluence-pipeline) via gh models CLI (${MODEL})"
  echo ""
